...
function AxiosDailyBoxOffice(props) {
  ...
  const loadBoxOffice = () => {
    ...
    axios.get('https://kobis.or.kr/kobisopenapi/webservice/rest/boxoffice/searchDailyBoxOfficeList.json?key=f5eef3421c602c6cb7ea224104795888&targetDt=20230719')
      ...
      .then(req => {
        // 통신 완료 후 전달받은 데이터 처리
        // 영화진흥원 open API의 일일 박스 오피스 정보 중 영화 목록만 가져옴
      ...
      <div className={'row'}>
        <div className={'col-sm mx-auto'}>
          <table className={'table table-hover table-stripped'}>
            ...
            <tbody>
            {
              // ES6의 JS 배열 관련 함수 중 map을 사용하여 배열의 요소를 반복 출력
              boxOffice.map(item => {
                return (
          ...
          <div className="my-3 d-flex justify-content-end">
            <button type={'button'} className={'btn btn-dark'} onClick={loadBoxOffice}>일일 박스오피스 조회</button>
          </div>
        </div>
      </div>
    </div>
