...
import Home from "./day7/pages/Home";
import About from "./day7/pages/About";
import ErrorPage from "./day7/pages/ErrorPage";
import Profile1 from "./day7/pages/Profile1";
import Profile2 from "./day7/pages/Profile2";

// react router : 리액트 프로젝트를 MPA 방식으로 사용할 수 있도록 해주는 라이브러리
// BrowserRouter : SPA 페이지를 MPA 페이지로 만들어 주는 객체. 가장 기본 라우터 개겣
// Routes : 페이지 객체를 여러개 가질 수 있는 객체
// Route : 화면에 그려지는 페이지 객체. Route 컴포넌트에 자식 컴포넌트로 Route 컴포넌트를 사용 시 부모 Route 컴포넌트의 주소를 기본 주소로 사용함
// path : 웹 브라우저에 표시되는 url 주소를 설정. 절대경로, 상대경로 다 사용 가능. * 을 사용 시 모든 페이지를 뜻함(path url을 정확하게 입력해야 접속이 가능. 주로 error 페이지 설정 시 사용함)
// element : path로 지정된 url 주소와 매칭되는 컴포넌트를 설정
// <Link> : 리액트 라우터에서 사용되는 링크 전용 컴포넌트. html의 a태그와 같은 역할
// to : 이동할 url 주소 입력. Route 컴포넌트의 path 속성에 설장한 값을 사용함

// URL 파라미터 : RestAPI 방식의 주소 형태로 구성되는 파라미터
// Route 컴포넌트의 path속성값 뒤에 /와 :파라미터명을 사용하여 데이터를 전달하는 방식
// rest 방식으로 구성되어 있을 경우 사용
// 연결된 컴포넌트에서 useParams 훅을 사용해야 함
// 사용법 :
//    기존url/:파라미터1/:파리미터2
//    <Route paht={url:/test1:/test2} element={<Url/>}/>
...
// useLocation() 훅을 사용하여 데이터를 가져오면 원하는 파라미터값을 가져오기 위해 직접 파싱 해야함
...
//    navigation(-1); // History를 이용하여 이전 페이지로 이동. 숫자를 음수로 입력시 이전 페이지로, 양수로 입력 시 다음 페이지로 이동
    ...
    <BrowserRouter>
      <div className={'container my-4'}>
        <Routes>
          ...
          {/* Route를 중첩했을 경우 자식 Route의 path 값의 시작 글자가 /가 있으면 안됨 */}
          <Route path={'/pages'}>
            ...
            {/* 쿼리스트링 방식으로 데이터 전달, 전통적인 get 방식으로 데이터 전달. url에 속성명=값 형태로 전달/*/}
