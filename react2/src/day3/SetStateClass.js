...
class SetStateClass extends React.Component {
  ...
  constructor(props) {
    ...
    // 자바스크립트의 생성자 안에 this.변수명을 사용 시 클래스멤버 변수로 동작함
    // SetStateClass의 멤버 변수로 state를 선언. 데이터 타입은 object 타입
  ...
  StateChange = (flag) => {
    if (flag === 'direct') {
      ...
      // 리액트는 가상돔을 사용하기 때문에 state의 내용을 직접 수정하면 수정된 내용이 가상돔에만 적용되고 실제 돔에 적용되지 않음
    ...
    if (flag === 'setState') {
      ...
      // setState()를 사용하여 내용을 수정 시 가상돔에 적용된 내용을 실제 돔에 적용함
      console.log(`원본 내용 : ${this.state.StateString}`);
  ...
  render() {
    return (
      <div>
        {/* JSX문법에서 문자열만 {}없이 사용 가능. {}가 사용 시 자바스크립트 코드임 */}
        {/* JSX문법에서 CSS 속성 사용시 '-' 기호를 생략하고 카멜명명법 사용 */}
        {/* JSX문법에서 클릭 이벤트가 onclick -> onClick으로 변경 */}
